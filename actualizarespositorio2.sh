...
#TODO #Añadir a las partes que trabajan con las listas de repositorios: #cat /etc/apt/sources.list |sort|uniq |tee /etc/apt/sources.list para eliminar lineas repetidas
#TODO: añadir: "sudo :" antes de un comando que necesite sudo, de forma que salga en la pantalla. Se debería poder elegir entre X o no (ver winetrick para ello) con gksudo
#TODO: Detectar guiones de descarga. Buscar archivos que ya están descargados y sustituirlos en tales guiones. Crear carpeta correspondiente y descargar faltantes.
...
--checklist "$COMENTARIO" "$ALTO" "$ANCHO" "$ALTOITEMSMENU" \
      A "Borra listas anteriores y crear nuevas" off \
      B "Mover antiguos a carpeta borrables" off \
      ...
      9 "Actualizar (descargar) listados de paquetes" off \
      10 "Activar todos los repositorios" off \
      11 "Actualizar paquetes" off \
...
damefile ()
{
#if [ ! -d "$1" -a ! -z "$2" ]; then sudo mkdir "$1"; fi #FIXME No funciona porque $2 es la lista

#Busca lo indicado por la lista
for i in $2; do
#FIXME: No está cuncinandno la discriminación de creació de directorios
#Si encuentra un elemento
if [ -e ./nuevos/$i* ]; then
...
#TODO: Arreglar arriba y activar esto para descargas.
#if [ ! -z `ls /var/cache/apt/archives/$i*` ]; then
# if [ ! -d "$1" ]; then sudo mkdir "$1"; fi
...
#Base, independientes del escritorio (o por lo menos existe en gnome y lxde).
#Apt deberá ir de último en la lista siempre
LISTA="sudo linux startupmanager dpkg alien debhelper gdm update-manager ifupdown intltool-debian mysql-common nfs ssh system-config html2text libpam libc6 zenity zip unzip p7zip-full synaptic unattended-upgrades ntfsprogs manpages mtools myspell gvfs hal dictionaries-common gksu deborphan gdebi localepurge parted gparted tzdata apt"
DIRE=base
damefile "$DIRE" "$LISTA"
...
#Lenguajes
LISTA="language-selector language-pack language-pack- language-pack-es"
DIRE=lenguajes
damefile "$DIRE" "$LISTA"

#Ligeros, de LXDE, etc
LISTA="mc pcmanfm leafpad lxdm lxrandr lxsession lxpanel openbox xarchiver obmenu obconf epiphany-browser"
DIRE=ligeros
damefile "$DIRE" "$LISTA"
...
#Utilidades
DIRE=utilidades
LISTA="ubuntu-tweak caffeine tucan ailurus grandr kupfer zsync terminator powertop meiga software-properties python-software-properties albumshaper"
damefile "$DIRE" "$LISTA"

#Utilidades borrables (no importantes)
DIRE=utilidadesborrables
LISTA="memaker ekiga gears planner netspeed tivion focuswriter usb-creator ubiquity"
damefile "$DIRE" "$LISTA"
...
LISTA="wicd python-iniparse python-wicd"
DIRE=wicd
damefile "$DIRE" "$LISTA"
...
#Xorg
LISTA="xserver x11 xorg xfonts xinit xinput xbitmaps xscreensaver xterm x-ttcidfont xvt"
DIRE=xorg
damefile "$DIRE" "$LISTA"
...
sudo fslint-gui ./ /var/cache/apt/archives/
